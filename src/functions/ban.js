...
module.exports = {
  ...
  func_func: async (message, args, funcObj, commandName) => {
    ...
    if (funcObj.bot || message.member.permissions.has("ADMINISTRATOR")) {
      try {
        if (!funcObj.bot) {
          await message.mentions.members.get(funcObj.member.split("@")[1].split(">")[0]).ban({ reason: funcObj.reason || args.slice(1).join(" ") });
        } else {
          await message.guild.members.cache.get(funcObj.member.split("@")[1].split(">")[0]).ban({ reason: funcObj.reason || args.slice(1).join(" ") });
        }
      } catch (err) {
        if (err.code === 50013) {

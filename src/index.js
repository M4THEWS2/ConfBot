...
// Import variables
const getVariables = require("../config/customVariables");
...
fs.readdirSync(path.join(__dirname, "functions/")).forEach((file) => {
    const funcObj = require(`./functions/${file}`);
    functions.set(funcObj.funcName, funcObj.funcFunc);
});
...
function replaceVariables(obj, message) {
    ...
    // If the value is a string replace the variables
        if (typeof value === "string") {
            ...
            replaceVariables(obj[key], message);
        }
    });
}
